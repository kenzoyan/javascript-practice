...
const texts = document.querySelector(".lorem-text");
...
form.addEventListener("submit",function(e){
  ...
  if (isNaN(value)||value<0 ||value>9){
    texts.innerHTML = `<p class="texts">${text[random]}</p>`;
  } else {
    let teptext = text.slice(0,value);
    teptext =teptext
    .map(function(item){
      return `<p class="texts">${item}</p>`
    })
    .join("");
    texts.innerHTML = teptext;
  }
})
